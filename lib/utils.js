...
const checkTargetScopeExists = (hrScope, targetScope, reducedUserScope, hierarchicalRoleScopingCheck) => {
    ...
    if (roleScopeEntExists && roleValueExists) {
        ...
        for (let role of userRoleAssocs) {
            if (role.role === ruleRoleValue) {
                ...
                if (userAssocHRScope && checkTargetScopeExists(userAssocHRScope, user.scope, reducedUserScope, hierarchicalRoleScopingCheck)) {
                    return true;
                }
            }
        }
...
const buildQueryFromTarget = (target, effect, userTotalScope, urns, userCondition, scopingUpdated, condition, reqSubject, database) => {
    ...
    if (!!scopingAttribute && effect == interfaces_1.Effect.PERMIT && !database) { // note: there is currently no query to exclude scopes
        ...
        query['filter'] = [];
        const filterParamKey = config_1.cfg.get('authorization:filterParamKey');
        console.log('Filter param key is..', filterParamKey);
